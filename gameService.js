...
export function registerHand(game, handData) {
  ...
  if (handData.type !== "EMPATE") {
    ...
    const points = Number(handData.handPoints);
  ...
  const newRound =
    calculateRound(updatedGame.hand);

  // ---------------------------------------
  // CAMBIO DE RONDA
  // ---------------------------------------

  if (newRound !== updatedGame.round) {

    updatedGame.round = newRound;
